...
export function HistoryPage() {
  const { history, activeTrackInfo } = useStudyApp()
    ...
    <main className="page history-page">
      <header className="page-heading">
        <div>
          ...
          <p>Reveja tentativas e acompanhe a consistência dos seus estudos em {activeTrackInfo.shortTitle}.</p>
        </div>
      ...
      {history.length === 0 ? (
        <section className="large-empty-state">
          ...
          <h2>Nenhum simulado em {activeTrackInfo.code}</h2>
          <p>Ao concluir um simulado nesta certificação, o resultado e todos os detalhes ficarão salvos aqui.</p>
              ...
              <article className="history-card" key={result.id}>
                ...
                <div className="history-card__main">
                  <div className="history-card__title">
                    <span className="exam-badge-tag">{result.track ?? activeTrackInfo.code}</span>
                    <span className="mode-badge">{modeLabels[result.mode] ?? result.mode}</span>
                ...
                <div className="history-card__score">
                  <span className={result.percentage >= (activeTrackInfo.passingPercentage ?? 65) ? 'score-good' : 'score-low'}>{formatPercent(result.percentage)}</span>
                  <div className="meter"><span className={result.percentage < (activeTrackInfo.passingPercentage ?? 65) ? 'meter--warning' : ''} style={{ width: `${result.percentage}%` }} /></div>
